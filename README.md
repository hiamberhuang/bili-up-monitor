...
## 🖥️ 两个页面
- **日常监控**：输入任意 UID → 近 N 条稿件播放 + 低播标记。
- **投流提报（每周低播）**：一键扫描全部签约作者（名册在 `index.html` 的 `SIGNED` 数组里改），窗口=上一个完整周（周日→周日），挑出低于该作者近 N 条均值的稿件，按投流申请表格式（提需日期/提需人/uid/昵称/投流稿件/申请原因/投流渠道/投流预算/备注/是否已投）一键复制粘进多维表格。
